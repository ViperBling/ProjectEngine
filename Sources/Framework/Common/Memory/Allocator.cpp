...
void ProjectEngine::Allocator::Reset(size_t data_size, size_t page_size, size_t alignment)
{
    ...
    PROJECTENGINE_ASSERT(alignment > 0 && ((alignment & (alignment-1))) == 0);
#endif
...
#if defined(PROJECTENGINE_DEBUG)
    DebugFillAllocatedBlock(freeBlock);
#endif
...
void ProjectEngine::Allocator::DebugFillFreeBlock(BlockHeader *pBlock)
{
    // block header + data
...
void ProjectEngine::Allocator::DebugFillAllocatedBlock(BlockHeader *pBlock)
{
    // block header + data
