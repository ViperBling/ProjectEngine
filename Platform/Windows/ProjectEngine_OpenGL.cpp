...
)
{
    ...
    // Qurey for a pixel format that fits the attributes we want
    result = wglChoosePixelFormatARB(g_deviceContext, attributeListInt, nullptr, 1, pixelFormat, &formatCount);
    if (result != 1) {
        return false;
    }

    // If the video carad can handle our desired pixel format then we set it as the current one
