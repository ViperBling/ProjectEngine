...
int ProjectEngine::TestApplication::Initialize()
{
    int result;
    ...
    if (result == 0)
    {
        ...
        buf = assetLoader.SyncOpenAndReadBinary("Textures/icelogo-normal.bmp");
        m_Image[1] = parser.Parse(buf);
    }
    return result;
}

void ProjectEngine::TestApplication::OnDraw()
{
...
void ProjectEngine::TestGraphicsManager::DrawBitmap(const Image *image, int32_t index)
{
    HRESULT hr;
    ...
    m_pRenderTarget->DrawBitmap(
            m_pBitmap,
            dest_rect, 1.0f,
            D2D1_BITMAP_INTERPOLATION_MODE_NEAREST_NEIGHBOR, source_rect);
